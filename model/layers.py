...
class GraphConvLayer(nn.Module):
    ...
    def forward(self, x):
        x_gc_in = self.align(x) # [32, 16, 10, 207]
        if self.graph_conv_type == 'cheb_graph_conv':
            x_gc = self.cheb_graph_conv(x_gc_in) #[32, 10, 207, 16]
        elif self.graph_conv_type == 'graph_conv':
...
class STConvBlock(nn.Module):
    ...
    def __init__(self, Kt, Ks, n_vertex, last_block_channel, channels, act_func, graph_conv_type, gso, bias, droprate):
        ...
        ################## Added part Jane 12/05/2023######################
        # Goal: Remember the initial input, initial x: [32, 1, 12, 207]
        self.Kt = Kt
        self.align = Align(last_block_channel, channels[2])
        ################## Added part Jane 12/05/2023######################
    ...
    def forward(self, x):
        ################## Added part Jane 12/05/2023######################
        # Goal: Remember the initial input, initial x: [32, 1, 12, 207]
        x_in = self.align(x)[:, :, 2*(self.Kt - 1):, :]
        ################## Added part Jane 12/05/2023######################
        x = self.tmp_conv1(x) #[32, 64, 10, 207]
        x = self.graph_conv(x) #[32, 16, 10, 207]
        x = self.relu(x)
        x = self.tmp_conv2(x) #[32, 64, 8, 207]
        ################## Added part Jane 12/05/2023######################
        # Goal: to add the residue from input to avoid the forgeting of initial temporal information
        x = x + x_in
        ####################################################################
